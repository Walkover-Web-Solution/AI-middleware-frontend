...
const useTutorialVideos = () => {
  ...
  const tutorialData = useCustomSelector((state) => 
    state?.flowDataReducer?.flowData?.tutorialData || []
  );
  ...
  const getTutorialVideo = (tutorialType) => {
    const tutorial = tutorialData?.find(tutorial => {
      ...
      switch (tutorialType) {
        ...
        case 'agent_creation':
          return title.includes('agent') || title.includes('bridge') || 
                 type === 'bridge_creation' || type === 'agent_creation';
                 
        case 'function_creation':
        case 'tool_creation':
          return title.includes('function') || title.includes('tool') || 
                 type === 'function_creation' || type === 'tool_creation';
                 
        case 'knowledge_base':
        case 'knowledgebase':
          return title.includes('knowledge') || title.includes('kb') || 
                 type === 'knowledge_base' || type === 'knowledgebase';
                 
        case 'parameter_setup':
        case 'advanced_parameter':
          return title.includes('parameter') || title.includes('config') || 
                 type === 'parameter_setup' || type === 'advanced_parameter';
                 
        case 'variable_management':
        case 'add_variables':
          return title.includes('variable') || title.includes('path') || 
                 type === 'variable_management' || type === 'add_variables';
                 
        case 'pauth_key':
        case 'api_key':
          return title.includes('pauth') || title.includes('key') || title.includes('auth') ||
                 type === 'pauth_key' || type === 'api_key';
                 
        case 'test_cases':
        case 'testing':
          return title.includes('test') || title.includes('case') || 
                 type === 'test_cases' || type === 'testing';
