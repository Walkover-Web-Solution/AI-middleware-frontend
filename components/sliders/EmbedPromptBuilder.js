...
const EmbedPromptBuilder = ({ configuration, onChange }) => {
  ...
  const [promptConfig, setPromptConfig] = useState(() => {
    ...
    if (typeof configPrompt === "object" && configPrompt !== null) {
      ...
      if (configPrompt.useDefaultPrompt === true || (!configPrompt.customPrompt && !configPrompt.role)) {
        return {
          useDefaultPrompt: true,
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {promptConfig.embedFields.map((field) => (
                  <div key={field.name} className="p-3 bg-base-100 rounded border border-base-300 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <label className="label cursor-pointer gap-1">
                          ...
                          />
                          <span className="label-text text-xs">{field.hidden ? "Hidden" : "Visible"}</span>
                        </label>
                    ...
                    {/* Field Value Input (shown when visible) */}
                    {/* Field Value Input (Hidden as per request) */}
                  </div>
                ))}
              </div>
            </div>
