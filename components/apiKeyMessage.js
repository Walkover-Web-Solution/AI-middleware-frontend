...
function ApiKeyMessage({ params }) {
  ...
  const bridgeApiKey = useCustomSelector((state) => {
    ...
    return state?.bridgeReducer?.bridgeVersionMapping?.[params?.id]?.[params?.version]?.apikey_object_id?.[service];
  });
  ...
  if (!bridgeApiKey) {
    return (
      <div
        ref={messageRef}
        className="absolute inset-0 flex flex-col items-center justify-center bg-base-200 z-[20] opacity-95 gap-2"
      >
