    ...
    <div key={`item-id-${item?.id}`} >
      {item?.role === "tools_call" ? (
        <div className="mb-2 flex flex-col justify-center items-center">
          <h1 className="auto p-2 mb-2 opacity-80 rounded-md flex items-center gap-2">
          ...
          <div className="w-full flex overflow-x-auto h-full gap-1 justify-center items-center whitespace-nowrap flex-wrap">
            {Object.entries(item?.tools_call_data?.[0] || {}).map(([key, tool], index) => (
              <div key={index} className="bg-base-200 rounded-lg w-1/7  flex gap-4 duration-200 items-center justify-between hover:bg-base-300 p-1">
