...
import ComparePromptModal from '@/components/modals/ComparePromptModal';

const InputConfigComponent = ({ params }) => {
  const { prompt: reduxPrompt, pulblishedPrompt, service, serviceType, variablesKeyValue } = useCustomSelector((state) => ({
    prompt: state?.bridgeReducer?.bridgeVersionMapping?.[params?.id]?.[params?.version]?.configuration?.prompt || "",
    pulblishedPrompt: state?.bridgeReducer?.allBridgesMap?.[params?.id]?.configuration?.prompt || " ",
  ...
  console.log("prompt",reduxPrompt ,"new prompt", pulblishedPrompt)
  ...
  return (
    <div>
        ...
        <div className='flex items-center justify-between gap-5 w-full'>
          ...
          <div className="flex items-center justify-between">
            <button
              className="label-text capitalize font-medium bg-gradient-to-r from-blue-800 to-orange-600 text-transparent bg-clip-text"
              onClick={() => {
                openModal(MODAL_TYPE?.COMPARE_PROMPT_MODAL);
              }}
            >
      ...
      <ComparePromptModal reduxPrompt={reduxPrompt} pulblishedPrompt={pulblishedPrompt}/>
    </div>
  );
};
