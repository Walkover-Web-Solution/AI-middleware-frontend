...
const InputConfigComponent = ({ params }) => {
  ...
  const getCaretCoordinatesAdjusted = () => {
    if (textareaRef.current) {
      ...
      const { selectionStart, scrollTop, scrollLeft, offsetTop, offsetLeft } =
        textarea;
  ...
  const handleKeyDown = useCallback(
    (e) => {
      ...
      const suggestionItems =
        suggestionListRef.current.querySelectorAll(".list-item");
  ...
  const handleSuggestionClick = useCallback(
    (suggestion) => {
      ...
      if (suggestion === "add_variable") {
        ...
        const isDoubleBrace =
          prompt.slice(cursorPosition - 2, cursorPosition) === "{{";
        const beforeCursor = isDoubleBrace
          ? prompt.slice(0, cursorPosition - 2)
          : prompt.slice(0, cursorPosition - 1);
  ...
  const renderSuggestions = () => {
    return (
      <div
        className="dropdown dropdown-open z-[999999]"
        style={{
          position: "absolute",
            ...
            <div className="text-xs">
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="">
                    - Use this variable if you are using the pre_function
                  </span>
                </div>
              </div>
            </div>
