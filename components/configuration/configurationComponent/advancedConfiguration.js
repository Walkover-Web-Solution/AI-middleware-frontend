...
const AdvancedConfiguration = ({ params, bridgeType, modelType }) => {
  ...
  return (
    <div className="z-very-low text-base-content w-full cursor-pointer mt-2" tabIndex={0}>
      ...
      <div className={`w-full gap-4 flex flex-col px-3 py-2 ${isAccordionOpen ? 'border-x border-b border-base-content rounded-x-lg rounded-b-lg' : 'border border-base-content rounded-lg'} transition-all duration-300 ease-in-out overflow-hidden ${isAccordionOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 p-0'}`}>
        {bridgeType === 'api' && modelType !== 'image' && modelType !== 'embedding' && (
          <div className="form-control w-full mt-2 border border-base-content rounded-lg p-2">
        ...
        <div className="form-control w-full">
          ...
          <div className="w-full">
            <div className="relative">
              <div
                className={`flex items-center gap-2 input input-bordered input-sm w-full border-base-content min-h-[2.5rem] cursor-pointer ${showApiKeysToggle ? 'rounded-x-md rounded-b-none rounded-t-md' : 'rounded-md'}`}
              ...
              {showApiKeysToggle && (
                <div className={`bg-base-100 border z-low max-h-80 overflow-y-auto p-2 transition-all ${showApiKeysToggle ? 'rounded-x-lg border-base-content border-t-0 rounded-t-none rounded-b-lg duration-300 ease-in-out' : ''}`}>
