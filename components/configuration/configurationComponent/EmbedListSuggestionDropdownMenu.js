...
function EmbedListSuggestionDropdownMenu({ params, searchParams, name, hideCreateFunction = false, onSelect = () => { }, onSelectPrebuiltTool = () => { }, connectedFunctions = [], shouldToolsShow, modelName, prebuiltToolsData, toolsVersionData, showInbuiltTools = {}, tutorialState, setTutorialState, isPublished = false, isEditor = true }) {
    ...
    const availablePrebuiltTools = useMemo(() => {
        ...
        return list.filter((t) => !selected.has(t.value) && (t?.name?.toLowerCase()?.includes(searchQuery?.toLowerCase() || '') && showInbuiltTools?.[t?.value]));
    ...
    return (
        <>
            ...
            {!tutorialState?.showTutorial && (
                <ul id="embed-suggestion-dropdown-menu" tabIndex={0} className="menu menu-dropdown-toggle dropdown-content z-high px-4 shadow bg-base-100 rounded-box w-72 max-h-96 overflow-y-auto pb-0">
                    <div className='flex flex-col gap-2 w-full'>
                        ...
                        {!hideCreateFunction && <li id="embed-suggestion-add-new-button" className="border-t border-base-300 w-full sticky bottom-0 bg-base-100 py-2" onClick={() => openViasocket(undefined,
                            {
                                embedToken,
                                meta: {
                                    createFrom: name,
