...
import {
  ADVANCED_BRIDGE_PARAMETERS,
  KEYS_NOT_TO_DISPLAY,
} from "@/jsonFiles/bridgeParameter";
import { updateBridgeVersionAction } from "@/store/action/bridgeAction";
import { MODAL_TYPE } from "@/utils/enums.js";
...
const AdvancedParameters = ({ params }) => {
  ...
  const { service, model, type, configuration } = useCustomSelector(
    (state) => ({
      service:
      ...
      type: state?.bridgeReducer?.bridgeVersionMapping?.[params?.id]?.[
        params?.version
      ]?.configuration?.type,
      configuration:
        state?.bridgeReducer?.bridgeVersionMapping?.[params?.id]?.[
  ...
  const { modelInfoData } = useCustomSelector((state) => ({
    modelInfoData:
      state?.modelReducer?.serviceModels?.[service]?.[type]?.[model]
        ?.configuration?.additional_parameters,
  }));
  ...
  const handleInputChange = (e, key, isSlider = false) => {
    ...
    if (e.target.type === "number") {
      newValue = String(newValue)?.includes(".")
        ? parseFloat(newValue)
  ...
  return (
    <div className="collapse text-base-content" tabIndex={0}>
      ...
      />
      <div
        className="collapse-title p-0 flex items-center justify-start font-medium cursor-pointer"
        onClick={toggleAccordion}
      >
        <span className="mr-2 cursor-pointer">Advanced Parameters</span>
                ...
                return (
                  <div key={key} className="form-control">
                    <label className="label">
                        ...
                        <div>
                          <ul className="menu menu-xs menu-horizontal lg:menu-horizontal bg-base-200 p-1 rounded-md text-xs">
                            {field === "slider" && (
                              <li>
                                <a
                                  onClick={() => setSliderValue("min", key)}
                                  className={
                                    configuration?.[key] === "min"
                            ...
                            <div
                              className="tooltip"
                              data-tip="If you set default, this key will not be send"
                            >
                              <li>
                                <a
                                  onClick={() => setSliderValue("default", key)}
                                  className={
                            ...
                            {field === "slider" && (
                              <li>
                                <a
                                  onClick={() => setSliderValue("max", key)}
                                  className={
                                    configuration?.[key] === "max"
                                      ? "bg-base-content text-base-100"
                                      : ""
                                  }
                                >
                      ...
                      {field === "slider" && (
                        <p
                          className={`text-right ${error ? "text-error" : ""}`}
                          id={`sliderValue-${key}`}
                        >
                          {configuration?.[key] === "min" ||
                          configuration?.[key] === "max" ||
                    ...
                    {field === "select" && (
                      <label className="items-center justify-start w-fit gap-4 bg-base-100 text-base-content">
                        ...
                        {configuration?.[key]?.type === "json_schema" && (
                          // <div
                          //   className="w-1/2 h-10 bg-red-300"
