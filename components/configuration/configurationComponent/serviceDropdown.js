...
function ServiceDropdown({ params, searchParams, apiKeySectionRef, promptTextAreaRef, isEmbedUser, isPublished }) {
    ...
    const renderServiceDropdown = () => (
        <Dropdown
            ...
            size="sm"
            className={`flex w-full items-center justify-between gap-2 rounded-md border px-3 py-2 text-sm whitespace-nowrap transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 border-white/[0.08] text-gray-100 h-10 focus:border-blue-500/50 min-w-[150px] ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            renderTriggerContent={({ selectedOption }) => {
              ...
              return (
                ...
                <span className="flex items-center gap-2">
                  <span
                    className="flex h-6 w-6 items-center justify-center rounded-md bg-base-200"
                  >
