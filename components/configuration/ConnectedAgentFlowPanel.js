  ...
  Object.entries(flowData).forEach(([agentId, agentData]) => {
    ...
    if (!bridgeInfo) {
      bridgeInfo = allBridges?.find(bridge => 
        bridge.versions && bridge.versions.includes(agentId)
      );
    }
...
const ConnectedAgentFlowPanel = () => {
  ...
  const { apiConnectedAgentFlow, bridgeData, allBridges } = useCustomSelector((state) => ({
    apiConnectedAgentFlow: state.orchestralFlowReducer.connectedAgentFlowByBridge?.[params?.org_id]?.[params?.id]?.[searchParams?.version] || null,
  ...
  useEffect(() => {
    if (currentView === 'agent-flow' && searchParams?.version) {
      dispatch(getConnectedAgentFlowAction({ 
        orgId: params.org_id,
        bridgeId: params.id,
