...
const Feature = () => {
  return (
    <div className="flex flex-col gap-4 w-3/4 mx-auto mt-20 z-10">
      {Array.from({ length: Math.ceil(FEATURE_DATA.length / 2) }, (_, rowIndex) => (
        <div key={rowIndex} className="flex flex-col md:flex-row items-stretch justify-between gap-4">
          {displayFeatures.slice(rowIndex * 2, rowIndex * 2 + 2).map((feature, index) => (
            <div key={index} className="flex-1 rounded-md p-10 flex flex-col justify-between border-opacity-40 border-gray-400 border-[0.1px] relative overflow-hidden group hover:z-0 hover:bg-zoom">
