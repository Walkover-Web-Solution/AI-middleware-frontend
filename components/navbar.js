      ...
      <div className={`sticky top-0 z-high transition-all duration-300 ${isScrolled
        ...
        <div className="flex items-center justify-between px-2 sm:px-4 py-2 sm:py-3 h-12 sm:h-14">
          ...
          <div className="flex items-center gap-1 sm:gap-3 min-w-0 flex-1">
            ...
            {/* Version Controls - show on configure tab for non-embed users - desktop only (1280px+) */}
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {!isEmbedUser && <button
              className={`btn btn-xs sm:btn-sm ${isMobile ? "flex-row" : "flex-col"} items-center gap-1 sm:gap-2 px-2 sm:px-3`}
              onClick={toggleIntegrationGuideSlider}
            >
              <FilterSliderIcon size={12} className="sm:w-3.5 sm:h-3.5"/>
              <span className="hidden sm:block text-xs sm:text-sm">Integration Guide</span>
            </button>}
        ...
        <div className="border-t border-base-200">
          <div className="px-1 h-8 sm:h-10">
            <div className="tabs tabs-lifted h-8 sm:h-10">
              {TABS.map(tab => (
                <button
                  key={tab.id}
                  onClick={() => handleTabChange(tab.id)}
                  className={`tab gap-1 sm:gap-2 h-8 sm:h-[42px] px-2 sm:px-4 ${activeTab === tab.id
