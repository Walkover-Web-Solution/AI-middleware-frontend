...
import AddEnvironmentVariable from "./addEnvirnomentVariable";
function Navbar() {
  const router = useRouter();
  const dispatch = useDispatch();
  const pathName = usePathname();
  const path = pathName.split("?")[0].split("/");
  const userdetails = useCustomSelector(
    (state) => state?.userDetailsReducer?.userDetails
  );
  const organizations = useCustomSelector(
    (state) => state.userDetailsReducer.organizations
  );
  const bridgesList =
    useCustomSelector((state) => state.bridgeReducer.org[path[2]]) || [];
  const bridgeData = useCustomSelector(
    (state) => state.bridgeReducer.allBridgesMap[path[5]]
  );
  const chatbotList =
    useCustomSelector((state) => state?.ChatBot?.org[path[2]]) || [];
  const chatbotData = useCustomSelector(
    (state) => state.ChatBot.ChatBotMap[path[5]]
  );
  ...
  const filteredBridgesList = bridgesList.filter((item) =>
  ...
  const filteredOrganizations = Object.values(organizations).filter((item) =>
  ...
  return (
    <div
      ...
      } w-full navbar bg-white border `}
    >
      <div className="flex items-center w-full justify-start gap-2">
        <button
          className="btn m-1"
          onClick={() => router.push(`/org/${path[2]}/${path[3]}`)}
        >
          <Home size={16} />
        </button>
        ...
        <div className="dropdown">
          ...
          >
            {["bridges", "chatbot", "apikey", "metrics", "invite"].map(
              (item) => (
                <li
                  key={item}
        ...
        {path[3] === "bridges" && path.length === 6 && (
          <button className="btn m-1" onClick={toggleBridgeSidebar}>
            {" "}
            {bridgeData?.service === "openai" ? (
              <svg
                width="24"
                height="25"
                viewBox="0 0 24 25"
        ...
        {path[3] === "chatbot" && path[4] === "configure" && (
          <button className="btn m-1" onClick={toggleChatbotSidebar}>
            {" "}
            <Rss size={16} /> {chatbotData?.title}{" "}
          </button>
        )}
      </div>
        ...
        {path.length === 6 && path[3] === "bridges" ? (
          <div className="flex items-center gap-3 ml-15">
            <div className="">
              ...
              {showAddVaribablePopup && (
                <AddEnvironmentVariable
                  id={path[5]}
                  setShowAddVariablePopup={setShowAddVariablePopup}
                />
              )}
            </div>
            ...
            <div className="join">
              <button
                onClick={() =>
                  router.push(`/org/${path[2]}/bridges/configure/${path[5]}`)
                }
                className={` ${
                  path[4] === "configure" ? "btn-primary" : ""
                }   btn join-item `}
              >
                {" "}
                <FileSliders size={16} /> Configure
              </button>
              <button
                onClick={() =>
                  router.push(`/org/${path[2]}/bridges/history/${path[5]}`)
                }
                className={` ${
                  path[4] === "history" ? "btn-primary" : ""
                }   btn join-item `}
              >
          ...
          <button
            className="btn btn-primary"
            onClick={() => document.getElementById("chatBot_model").showModal()}
          >
            + create new chatbot
          </button>
        ) : (
      ...
      <div
        ref={sidebarRef}
        className={`fixed inset-y-0 left-0 border-r-2 ${
          isOpen ? "w-full md:w-1/3 lg:w-1/6 opacity-100" : "w-0"
              ...
              {filteredOrganizations
                ...
                .map((item) => (
                  <li key={item.id}>
                    <a
                      className={`${
                        item.id == path[2] ? "active" : `${item.id}`
                      } py-2 px-2 rounded-md`}
                      key={item.id}
                      onClick={() => {
                        handleSwitchOrg(item.id, item.name);
                        router.push(`/org/${item.id}/${path[3]}`);
                      }}
                    >
                      {" "}
            ...
            <details className="overflow-hidden rounded border border-gray-300 [&_summary::-webkit-details-marker]:hidden">
              ...
              <div className="border-t border-gray-200 bg-white">
                <ul className="menu w-full   text-base-content">
                  <li>
                    {" "}
                    <a className="py-2 px-2 rounded-md">
                      {" "}
                  ...
                  <li>
                    ...
                    <a
                      className={`py-2 px-2  ${
                        path[3] === "apikey" ? "active" : ""
                      }  rounded-md`}
                      onClick={() => {
                        router.push(`/org/${path[2]}/apikey`);
                        setIsOpen(false);
                      }}
                    >
                      {" "}
      ...
      <div
        ref={sideBridgeRef}
        className={`fixed inset-y-0 left-0 border-r-2 ${
          isOpenBridge ? "w-full md:w-1/3 lg:w-1/6 opacity-100" : "w-0"
        } overflow-y-auto border-l bg-base-200 transition-all duration-300 z-50`}
      >
        <aside className="flex w-full  flex-col h-screen overflow-y-auto">
              ...
              {filteredBridgesList
                ...
                .map((item) => (
                  <li key={item._id} className="max-w-full">
                    <a
                      className={`  ${
                        item._id == path[5] ? "active" : `${item.id}`
                      } py-2 px-2 rounded-md truncate max-w-full`}
                      onClick={() =>
                        router.push(
                          `/org/${path[2]}/bridges/configure/${item._id}`
                        )
                      }
                    >
                      {item.service === "openai" ? (
      ...
      <div
        ref={sideBotRef}
        className={`fixed inset-y-0 left-0 border-r-2 ${
          isOpenChatbot ? "w-full md:w-1/3 lg:w-1/6 opacity-100" : "w-0"
        } overflow-y-auto border-l bg-base-200 transition-all duration-300 z-50`}
      >
        <aside className="flex w-full flex-col h-screen overflow-y-auto">
                ...
                .map((item) => (
                  <li key={item._id} className=" w-full">
                    <a
                      ...
                      onClick={() =>
                        router.push(
                          `/org/${path[2]}/chatbot/configure/${item._id}`
                        )
                      }
                    >
                      {item?.title}
