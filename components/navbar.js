...
import { logoutUserFromMsg91, switchOrg, switchUser } from '@/api';
  ...
  return (
    <div className={` ${router.pathname === '/' ? 'hidden' : 'flex items-center justify-between '} navbar bg-white border `}>
      <div className='flex items-center justify-center gap-2'>
        ...
        {path.length === 5 && <button className="btn m-1" onClick={toggleBridgeSidebar}> <Rss size={16} /> {chatbotData?.title} </button>}
