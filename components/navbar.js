...
function Navbar() {
  ...
  return (
    <div className="flex items-center justify-between w-full navbar bg-base-100 border-b">
      <div className="flex items-center justify-start gap-2">
        ...
        <div className="dropdown">
          <button tabIndex={0} role="button" className="capitalize btn  m-1"> <div className='flex gap-2 w-fit' > {path[3]} <ChevronDown size={16} /></div> </button>
      ...
      <div className="justify-end w-fit" >
        {path.length === 6 && path[3] === 'bridges' ? (
          <div className="join">
            <button
              onClick={() => router.push(`/org/${path[2]}/bridges/configure/${path[5]}`)}
              className={` ${path[4] === 'configure' ? 'btn-primary' : ''
                } btn join-item`}
            >
              <FileSliders size={16} /> Configure
            </button>
            <button
              onClick={() => router.push(`/org/${path[2]}/bridges/history/${path[5]}`)}
              className={` ${path[4] === 'history' ? 'btn-primary' : ''
                } btn join-item`}
            >
      ...
      <div
        ref={sidebarRef}
        className={` inset-y-0 left-0 border-r-1 ${isOpen
          ? 'w-full fixed md:w-1/3 lg:w-1/6 opacity-100'
          : 'w-0 hidden'
          } overflow-y-auto border-l bg-base-200 transition-all duration-300 z-50`}
      >
        <aside className="flex flex-col h-screen w-full overflow-y-auto">
          <div className="p-4 flex flex-col gap-4">
            ...
            <ul className="menu p-0 w-full text-base-content">
              {filteredOrganizations.length > 0 ? (
                  ...
                  .map((org) => (
                    <li key={org.id}>
                      <a
                        ...
                        className={`${path[2] === org.id ? 'font-semibold text-primary' : ''
                          } capitalize`}
                      >
                        {org.name}
                      </a>
                ...
                <li>No data present for this search</li>
              )}
            </ul>
          </div>
      ...
      <div
        ref={sideBridgeRef}
        className={` inset-y-0 left-0 border-r-1 ${isOpenBridge
          ? 'w-full fixed md:w-1/3 lg:w-1/6 opacity-100'
          : 'w-0 hidden'
          } overflow-y-auto border-l bg-base-200 transition-all duration-300 z-50`}
      >
        <aside className="flex flex-col h-screen overflow-y-auto w-full">
          <div className="p-4 flex flex-col gap-4">
            <p className="text-xl">Bridge</p>
            <input
              type="text"
              placeholder="Search..."
            ...
            <ul className="menu p-0 w-full text-base-content">
              {filteredBridgesList.length > 0 ? (
                filteredBridgesList.map((bridge) => (
                  <li key={bridge._id}>
                    <a
                      onClick={() => {
                        toggleBridgeSidebar();
                        router.push(
                          `/org/${path[2]}/bridges/configure/${bridge._id}`
                        );
                      }}
                      className={`${path[5] === bridge.id ? 'font-semibold text-primary' : ''
                        } capitalize truncate`}
                      title={bridge.name} // Tooltip to show full name on hover
                    >
                      {bridge.service === 'openai' ?
      ...
      < div
        ref={sideBotRef}
        className={` inset-y-0 left-0 border-r-1 ${isOpenChatbot
          ? 'w-full fixed md:w-1/3 lg:w-1/6 opacity-100'
          : 'w-0 hidden'
          } overflow-y-auto border-l bg-base-200 transition-all duration-300 z-50`}
      >
        <aside className="flex w-full flex-col h-screen overflow-y-auto">
          <div className="p-4 flex flex-col gap-4">
            ...
            />
            <ul className="menu p-0 w-full text-base-content">
              {filteredChatbotsList.length > 0 ? (
                filteredChatbotsList.map((chatbot) => (
                  <li key={chatbot.id}>
                    <a
                      className={`${chatbot._id == path[5] ? "active" : `${chatbot.id}`} py-2 px-2 rounded-md`}
                      onClick={() => router.push(`/org/${path[2]}/chatbot/configure/${chatbot._id}`)}
                    >
                      {chatbot.title}
                    </a>
                  </li>
