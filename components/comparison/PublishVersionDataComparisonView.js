...
const PublishVersionDataComparisonView = ({ oldData, newData, params }) => {
  ...
  // Preprocess data to transform arrays like embedFields into objects for value comparisons
  const preprocessData = (data) => {
    if (!data) return data;
    const cloned = JSON.parse(JSON.stringify(data));

    // Helper to traverse and transform
    const traverseAndTransform = (obj) => {
      if (!obj || typeof obj !== "object") return;

      // Check if prompt is a string
      // If it is, convert it to an object with 'instruction' key (or matching field)
      // to allow granular comparison with new structured prompt
      if (obj.prompt && typeof obj.prompt === "string") {
        try {
          // Try to parse if it's a JSON string
          const parsedPrompt = JSON.parse(obj.prompt);
          obj.prompt = parsedPrompt;
        } catch {
          // If not JSON, treat the entire string as the 'instruction' field
          // This aligns with user logs where old textual prompt matches the 'instruction' field of the new object
          obj.prompt = {
      ...
      // Transform prompt.embedFields array to object
      if (obj.prompt && typeof obj.prompt === "object" && Array.isArray(obj.prompt.embedFields)) {
        const fieldsObj = {};
        obj.prompt.embedFields.forEach((field) => {
          if (!field.hidden && field.name) {
            fieldsObj[field.name] = field.value || "";
          }
        });
        obj.prompt.embedFields = fieldsObj;
      ...
      // Also handle root-level config if prompt is nested deeper
            ...
            <div key={category} className="mb-6">
              ...
              <div className="space-y-4">
                {items.map(({ path, oldValue, newValue, status }) => {
                  // Check if this is the prompt field or its sub-fields
                  // We also include anything inside embedFields object (generated by preprocessData)
                  const isPromptField =
                    path === "prompt" ||
