...
function CreateNewBridge({ orgid }) {
  ...
  const createBridgeHandler = (name, slugname) => {
    if (name.length > 0 && selectedModel && selectedType) {
      ...
      dispatch(
        createBridgeAction({ dataToSend: dataToSend, orgid }, (data) => {
          console.log("success", dataToSend, orgid, data);

          // setShowFileUploadModal(false);
          route.push(
            `/org/${orgid}/bridges/configure/${data.data.bridge._id}?version=${data.data.bridge.versions[0]}`
          );
          setIsLoading(false);
  ...
  const cleanState = () => {
    ...
    if (document.getElementById("slug-name"))
            ...
            <div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <label className="flex items-center justify-center gap-2">
                    <input
                      ...
                      className="radio"
                      value="chatbot"
                      defaultChecked={bridgeType === "chatbot"}
              ...
              <label className="form-control w-full mb-2">
                ...
                <input
                  type="text"
                  id="bridge-name"
                  defaultValue={allBridgeLength === 0 ? "Assistant" : ""}
                        ...
                        return (
                          <optgroup label={group} key={`group_${groupIndex}`}>
                            {Object.keys(options || {}).map(
                              (option, optionIndex) => {
                                const modelName =
                                  options?.[option]?.configuration?.model
                                    ?.default;
                                return (
                                  <option
              ...
              {bridgeType === "chatbot" ? (
                <label className="form-control w-full mb-2">
                  ...
                  <input
                    type="text"
                    id="slug-name"
                    defaultValue={allBridgeLength === 0 ? "root" : ""}
                    placeholder="Type here"
                    className="input input-bordered w-full "
                    required={bridgeType === "chatbot"}
                  />
                </label>
              ) : null}
            </div>
            <div className="modal-action">
              <button className="btn" onClick={cleanState}>
                Close
              </button>
