...
const Page = ({ params }) => {
  ...
  const knowledgeBaseToken = useCustomSelector((state) => state?.knowledgeBaseReducer?.knowledgeBaseData?.[params?.org_id]?.knowledgeBaseToken);
  ...
  console.log('knowledgeBaseToken',knowledgeBaseToken)
  useEffect(() => {
  ...
  useEffect(() => {
      const updateScript = (knowledgeBaseToken) => {
        const existingScript = document.getElementById("rag-main-script");
        if (existingScript) {
          document.head.removeChild(existingScript);
        }
        if (knowledgeBaseToken) {
          const script = document.createElement("script");
          script.setAttribute("embedToken", knowledgeBaseToken);
          script.setAttribute("parentId",'rag_container');
     ...
     updateScript(knowledgeBaseToken)
      // return () => {
  ...
  return (
    ...
    // </div>
    <>
    <div id="rag_container" className="h-[92vh]">
    </div>
    </>
  );
};
