...
import {
  createWebhookAlertAction,
  deleteWebhookAlertAction,
  getAllWebhookAlertAction,
  updateWebhookAlertAction,
} from "@/store/action/webhookAlertAction";
...
const WebhookPage = ({ params }) => {
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      if (
        !form.id.value &&
        webhookAlertData.some((alert) => alert.name === form.name.value)
      ) {
  ...
  const handleEdit = (item) => {
    ...
    if (formRef.current) {
      ...
      setSelectedAlertTypes(
        Array.isArray(item.alertType) ? item.alertType : []
      );

      formRef.current.id.value = item._id;
    }

    const bridgeIds = item.bridges.includes("all")
      ? filteredBridges.map((b) => b._id)
      : item.bridges;
  ...
  const renderForm = () => (
    <dialog
      ref={modalRef}
      id={MODAL_TYPE?.WEBHOOK_MODAL}
      className="w-[60%] h-[80%] p-6 rounded-lg "
    >
          ...
          <div className="relative">
            <div
              className={`flex justify-start items-center ${
                selectedBridges.length === 0 ? "mb-2" : "mb-0"
              }`}
            >
              <label className="label">
                <span className="label-text">
                  Select Bridges For Getting Alert
                </span>
              </label>
            ...
            >
              {selectedBridges.length > 0
                ? `Selected Bridges: ${selectedBridges.length}`
                : "Select the bridge"}{" "}
              <span className="text-end">
                <ChevronDown />
              </span>
