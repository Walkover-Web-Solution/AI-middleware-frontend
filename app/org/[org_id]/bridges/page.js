...
function Home({ params }) {
  ...
  ); // State to manage view mode based on screen size
  ...
  const filteredArchivedBridges = filteredBridges?.filter(
    (item) => item.status === 0
  );
  const filteredUnArchivedBridges = filteredBridges?.filter(
    (item) => item.status === 1 || item.status === undefined
  );

  const UnArchivedBridges = filteredUnArchivedBridges
      ...
      name: (
        <div className="flex gap-3">
          ...
          <div className="flex-col">
            {item.name}
            <p className="opacity-60 text-xs">{item?.slugName || ""}</p>
          </div>
        </div>
      ),
  ...
  const ArchivedBridges = filteredArchivedBridges
  ...
  const onClickConfigure = (id, versionId) => {
    router.push(
      `/org/${params.org_id}/bridges/configure/${id}?version=${versionId}`
    );
  };
      ...
      <div className="flex rounded-md border cursor-pointer hover:shadow-lg bg-base-100 p-4 relative w-full">
        <div
          ...
          onClick={() =>
            onClickConfigure(
              item._id,
              item?.published_version_id || item?.versions?.[0]
            )
          }
        >
          <div className="flex flex-col h-[200px] gap-2 w-full">
            ...
            <p className="text-xs w-full flex items-center gap-2 line-clamp-5">
              ...
              {item.configuration?.input && (
                <span>Input: {item.configuration.input}</span>
              )}
            </p>
        ...
        <div className="dropdown bg-transparent absolute right-3 top-2">
          <div
            tabIndex={0}
            role="button"
            className="hover:bg-base-200 rounded-lg p-3"
            onClick={(e) => e.stopPropagation()}
          >
            <Ellipsis className="rotate-90" size={16} />
          </div>
          <ul
            tabIndex={0}
            ...
            <li>
              <a
                onClick={(e) => {
                  ...
                  archiveBridge(
                    item._id,
                    item.status != undefined ? Number(!item?.status) : undefined
                  );
                }}
              >
                {item?.status === 0 ? "Un-archive Bridge" : "Archive Bridge"}
              </a>
            </li>
          </ul>
        </div>
  ...
  const archiveBridge = (bridgeId, newStatus = 0) => {
    try {
      dispatch(archiveBridgeAction(bridgeId, newStatus)).then(
        (bridgeStatus) => {
          if (bridgeStatus === 1) {
          ...
          <li>
            <a
              onClick={(e) => {
                ...
                archiveBridge(
                  row._id,
                  row.status != undefined ? Number(!row?.status) : undefined
                );
              }}
            >
              {row?.status === 0 ? "Un-archive Bridge" : "Archive Bridge"}
            </a>
          </li>
        </ul>
      </div>
              ...
              >
                  ...
                  <div className="join hidden sm:block">
                    <a
                      ...
                      className={`btn join-item ${
                        viewMode === "grid" ? "bg-primary text-base-100" : ""
                      }`}
                    >
                    ...
                    <a
                      ...
                      className={`btn join-item ${
                        viewMode === "table" ? "bg-primary text-base-100" : ""
                      }`}
                    >
                ...
                {viewMode === "grid" ? (
                  ...
                  <CustomTable
                    ...
                    columnsToShow={["name", "prompt", "model"]}
                    sorting
                    sortingColumns={["name", "model"]}
                    handleRowClick={(props) =>
                      onClickConfigure(props?._id, props?.versionId)
                    }
