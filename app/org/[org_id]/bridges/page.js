...
function Home({ params }) {
  const allBridges = useCustomSelector(
    (state) => state.bridgeReducer.org[params.org_id] || []
  ).slice().reverse();
  ...
  const filteredBridges = allBridges.filter(
    (item) =>
  ...
  const handleOpenModal = (bridgeId) => {
            ...
            {allBridges.length === 0 ? (
              <div className="text-center w-full h-screen flex justify-center items-center py-10">
                <div className="flex flex-col items-center justify-center space-y-4">
                  ...
                  <button
                    className="btn mt-2 mr-3 btn-primary"
              ...
              <div className="flex flex-col">
                <div className="relative flex items-center justify-between mx-4">
                  ...
                  />
                  <button
                    className="btn w-fit float-start m-4 btn-primary"
                    onClick={() => router.push(`/org/${params.org_id}/metrics`)}
                  >
                    <Box size={16} /> Metrics
                  </button>
                </div>
                  ...
                  {filteredBridges
                    ...
                    .map((item) => (
                      <div
                        key={item._id}
                        data-id={item._id}
                        className="flex flex-col items-center gap-7 rounded-md border cursor-pointer hover:shadow-lg"
                        onClick={() => onClickConfigure(item._id)}
                      >
                        <div className="w-full">
                          <div className="p-4 flex flex-col justify-between h-[200px] items-start">
                            <div className="w-full">
                              <h1 className="inline-flex truncate w-full items-center gap-2 text-lg font-semibold">
                                <svg
                                  width="24"
                                  height="25"
                                  viewBox="0 0 24 25"
                              ...
                              <p className="text-xs w-full flex items-center gap-2 text-gray-600 line-clamp-5">
                                ...
                                {item?.["configuration"]?.["input"] && (
                                  <p className="text-xs">
                                    Input: {item["configuration"]["input"]}
                                  </p>
                                )}
                              </p>
                            </div>
