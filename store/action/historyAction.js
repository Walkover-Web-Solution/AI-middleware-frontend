import { getHistory, getSingleThreadData, getSubThreadIds, getThreads, updateHistoryMessage, userFeedbackCount, getRecursiveHistory } from "@/config";
import {
  ...
  fetchRecursiveHistoryError,
} from "../reducer/historyReducer";

export const getHistoryAction = (id, page = 1, user_feedback, isErrorTrue, selectedVersion, keyword, startDate, endDate) => async (dispatch) => {
  try {
...
export const getRecursiveHistoryAction = ({ agent_id, thread_id, message_id }) => async (dispatch) => {
  try {
    ...
    dispatch(fetchRecursiveHistoryError({ error: error.message }));
    throw error;
  }
}
