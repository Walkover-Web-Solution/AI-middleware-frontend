...
export const getSingleBridgesAction = (id) => async (dispatch, getState) => {
  try {
    ...
    getBridgeVersionAction({
      versionId:
        data.data?.bridge?.published_version_id ||
        data.data?.bridge?.versions?.[0],
    })(dispatch);
  } catch (error) {
    dispatch(isError());
    console.error(error);
...
export const createBridgeAction =
  (dataToSend, onSuccess) => async (dispatch, getState) => {
    try {
...
export const createBridgeVersionAction =
...
export const getAllBridgesAction = (onSuccess) => async (dispatch) => {
  try {
    ...
    dispatch(
      fetchAllBridgeReducer({
        bridges: response?.data?.bridge,
        orgId: response?.data?.org_id,
        integrationData: flowObject,
        embed_token,
      })
    );
  } catch (error) {
...
export const getAllResponseTypesAction =
  (orgId) => async (dispatch, getState) => {
    try {
      dispatch(isPending());
...
export const updateBridgeVersionAction =
  ...
  async (dispatch) => {
    try {
      dispatch(isPending());
      console.log("Calling API with:", { versionId, dataToSend });
...
export const createApiAction = (org_id, dataFromEmbed) => async (dispatch) => {
  try {
    ...
    if (data?.success) {
      dispatch(
        updateBridgeToolsReducer({ orgId: org_id, functionData: data?.data })
      );
    }
  } catch (error) {
    console.error(error);
...
export const updateApiAction =
  (bridge_id, dataFromEmbed) => async (dispatch) => {
    try {
...
export const addorRemoveResponseIdInBridgeAction =
  (bridge_id, org_id, responseObj) => async (dispatch) => {
    try {
      const response = await addorRemoveResponseIdInBridge(
        bridge_id,
        org_id,
        responseObj
      );
...
export const getChatBotOfBridgeAction =
  (orgId, bridgeId) => async (dispatch) => {
    try {
...
export const archiveBridgeAction =
  ...
  async (dispatch) => {
    try {
      ...
      dispatch(
        updateBridgeReducer({ bridges: response.data, functionData: null })
      );
