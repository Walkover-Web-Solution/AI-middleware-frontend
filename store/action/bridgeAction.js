...
export const getAllBridgesAction = (onSuccess) => async (dispatch) => {
  try {
    ...
    const docStar_prompt_embed_token=response?.data?.docStar_prompt_embed_token;


    if (onSuccess) onSuccess(response?.data?.bridge?.length)
    dispatch(fetchAllBridgeReducer({ bridges: response?.data?.bridge, orgId: response?.data?.org_id, embed_token,docStar_prompt_embed_token, alerting_embed_token, history_page_chatbot_token, triggerEmbedToken, average_response_time }));
