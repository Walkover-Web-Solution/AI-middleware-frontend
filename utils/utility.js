...
export const updatedData = (obj1, obj2 = {}, type) => {
  ...
  if (type === "chat") {
    const inputconfig = updateContent(
      obj2.inputConfig,
      updatedObj1.configuration
    );
  ...
  const newObj1 = removeDuplicateFields(
    obj1.configuration,
    updatedObj1.configuration
  );
...
export function flattenParameters(parameters, prefix = "") {
  ...
  Object.entries(parameters || {}).forEach(([key, value]) => {
    ...
    if (Object?.keys(value?.parameter || value?.items || {})?.length > 0) {
      flat = flat?.concat(
        flattenParameters(
          value?.parameter || value?.items?.properties || {},
          currentKey
        )
      );
    }
...
export function filterBridges(bridgesList, bridgeSearchQuery) {
  return bridgesList.filter(
    (item) =>
      ...
      item?.slugName
        ?.toLowerCase()
        ?.includes(bridgeSearchQuery.toLowerCase()) ||
