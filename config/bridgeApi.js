...
export const getSingleBridge = async (bridgeId) => {
  try {
    const response = await axios.get(`${URL}/api/v1/agentConfig/getbridge/${bridgeId}`)
    return response
  } catch (error) {
    if (error.response) {
...
export const getAllBridges = async () => {
  try {
    const data = await axios.get(`${URL}/api/v1/agentConfig/getbridges/all`)
    return data;
  } catch (error) {
    console.error(error)
...
export const createBridge = async (dataToSend) => {
  try {
    return await axios.post(`${URL}/api/v1/agentConfig/create_bridge`, dataToSend)
  } catch (error) {
...
export const updateBridge = async ({ bridgeId, dataToSend }) => {
  try {
    const response = await axios.post(`${URL}/api/v1/agentConfig/update_bridge/${bridgeId}`, dataToSend);
    return response
  } catch (error) {
    console.error(error)
    toast.error(error?.response?.data?.error);
...
export const deleteBridge = async (bridgeId, org_id, restore = false) => {
  try {
    const response = await axios.delete(`${URL}/api/v1/agentConfig/deletebridges/${bridgeId}`, { data: { org_id, restore } });
    return response;
  } catch (error) {
    console.error(error);
    throw error;
...
export const archiveBridgeApi = async (bridge_id, newStatus) => {
  try {
    const response = await axios.put(`${URL}/api/v1/agentConfig/bridge-status/${bridge_id}`, { status: newStatus });
    return response.data;
  } catch (error) {
    console.error(error);
    throw new Error(error);
...
export const createBridgeWithAiAPi = async ({ ...dataToSend }) => {
  try {
    ...
    const response = await axios.post(`${URL}/api/v1/agentConfig/create_bridge`, dataToSend);
    return response;
  } catch (error) {
    console.error(error);
    return error;
...
export const getBridgeConfigHistory = async (versionId, page = 1, pageSize = 30) => {
  try {
    const response = await axios.get(`${URL}/api/v1/agentConfig/getuserupdates/${versionId}?page=${page}&limit=${pageSize}`);
    return response.data;
  } catch (error) {
