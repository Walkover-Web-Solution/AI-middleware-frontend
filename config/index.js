...
export const getSingleModels = async () => {
  try {
    const response = await axios.get(`${URL}/api/v1/config/models`);
    return response;
  } catch (error) {
    console.error(error);
...
export const getSingleThreadData = async (threadId, bridgeId) => {
  try {
    const response = await axios.get(`${URL}/api/v1/config/threads/${threadId}/${bridgeId}`);
    return response;
  } catch (error) {
    console.error(error);
  }
...
export const getHistory = async (bridgeId) => {
  try {
    const response = await axios.get(`${URL}/api/v1/config/history/${bridgeId}`);
    return response;
  } catch (error) {
    console.error(error);
  }
...
export const logout = async () => {
  try {
    await axios.delete(`${PROXY_URL}/{featureId}/deleteCCompany/{cCompanyId}`);
  } catch (error) {
...
export const allAuthKey = async () => {
  try {
    const response = await axios.get(`${PROXY_URL}/api/c/authkey`);
